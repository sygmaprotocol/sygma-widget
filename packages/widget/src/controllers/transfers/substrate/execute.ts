...
export async function executeNextSubstrateTransaction(
  ...
  const unsub = await this.transferTx?.signAndSend(address, ({ events = [], status }) => {
    if (status.isInBlock) {
      console.log('Successful transfer of', this.resourceAmount, 'with hash', status.asInBlock.toHex());
